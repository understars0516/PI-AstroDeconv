...
class unet2D(): 
    ...
    def build_model(self):
        ...
        last_fft = tf.multiply(fft2d(tf.cast(last_layer, dtype=tf.complex64)), fft2d(tf.cast(self.psf, dtype=tf.complex64)))
        output = tf.math.real(fftshift(ifft2d(last_fft)))
