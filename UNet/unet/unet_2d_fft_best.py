...
class unet2D(): 
    def __init__(self,n_filters = 16, conv_width=1, 
        ...
        self.dirty_min = dirty_min
        self.dirty_max = dirty_max
        self.psf = psf
    ...
    def build_model(self):
        ...
        last_layer = Conv2DTranspose(self.n_channels,1,padding="same",name="last_layer", dtype=tf.float64)(x)
        #layer = MinMaxNorm(min_value = self.dirty_min, max_value = self.dirty_max, rate = 0)
        #last_layer = layer(last_layer)
        #last_layer = tf.clip_by_value(last_layer, self.dirty_min, self.dirty_max)
#        last_layer = tf.cast(last_layer, dtype=np.float64)
        psf = tf.cast(self.psf, dtype=np.float64) 
